...
FROM ( 
    SELECT *,
           row_number() OVER (ORDER BY replacement_cost ASC) as rc
    FROM film
) t
WHERE rc <= 10;
...
--     the amount and the date of payment ordered by the id of the staff 
--     member who sold them the dvd.
SELECT CONCAT(staff.first_name, ' ', staff.last_name) as staff,  
